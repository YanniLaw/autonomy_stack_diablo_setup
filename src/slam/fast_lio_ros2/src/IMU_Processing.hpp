...
class ImuProcess
{
 public:
  ...
  bool IsIMUInited() {return !imu_need_init_;};
  Eigen::Quaterniond GetGravityIMUQuaternion() {return q_gravity_imu_;};
 ...
 private:
  ...
  V3D angvel_last;
  V3D acc_s_last;
  Eigen::Quaterniond q_gravity_imu_;
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  // 重力对齐
  std::cout << "mean_acc: " << mean_acc.transpose() << ", mean_gyro: " << std::endl;
  q_gravity_imu_ = Eigen::Quaterniond::FromTwoVectors((-mean_acc).normalized(), Eigen::Vector3d(0.0, 0.0, -1.0));
  std::cout << "q_gravity_imu_: " << q_gravity_imu_ << std::endl;
  if (true) {
    init_state.rot = q_gravity_imu_;
    std::cout << "init_state.rot " << init_state.rot << std::endl;
    // init_state.ba = acc_bias*G_m_s2;
